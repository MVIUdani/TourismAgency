...
import Count_Validation from './packages/count/count_validation';
...
function Count_pasenger(){
    ...
    const onSubmit=(data)=>{
         Axios.post("http://localhost:5000/insertCount_p1",data,{
            count: data.count,       //10
            room_count: data.room_count, //3
        }).then((response) => {
            console.log(response);
          });
          history.push('/payment-first');
    };
...
/*function Count_pasenger(){
    ...
    const addCount=()=>{
        Axios.post("http://localhost:5000/insertCount_p1",{
            count: count,       //10
            room_count: room_count, //3
        }).then((response) => {
            console.log(response);
          });
          history.push('/payment-first');
