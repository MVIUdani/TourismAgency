...
function Count_pasenger_four(){

    const history = useHistory();

    const onSubmit=(data)=>{
        Axios.post("http://localhost:5000/insertCount_p4",data,{
            count: data.count, //12
            room_count: data.room_count, //4
        }).then((response) => {
            console.log(response);
          });
          history.push('/payment-first-Pfour');
    };
...
export default Count_pasenger_four;
